...
func (f FixedLength) EncodeTo(out []byte) (int, error) {
	...
	_n += 1
	// Time
	f_Time, err := _timeMarshalBinary(f.Time, out[_n:])
	if err != nil {
		return 0, err
	}
	_n += f_Time
...
func (u Unnamed) EncodeTo(out []byte) (int, error) {
	...
	// Time
	u_Time, err := _timeMarshalBinary(u.Time, out[_n:])
	if err != nil {
		return 0, err
	}
	_n += u_Time
...
func (o OtherPackage) EncodeTo(out []byte) (int, error) {
	...
	if o.TestStruct == nil {
		...
		out[_n] = 1
		_n += 1
		ƒço_TestStructå, err := (*o.TestStruct).EncodeTo(out[_n:])
		if err != nil {
			return 0, err
		}
		_n += ƒço_TestStructå
	...
	// StrAlias
	o_StrAlias, err := o.StrAlias.EncodeTo(out[_n:])
	if err != nil {
		return 0, err
	}
	_n += o_StrAlias
